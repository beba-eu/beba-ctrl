	...
	def install_leaves(self, datapath): 
		...
		for i in UPPER_PORTS:
			...
			actions_ewma_1 = [#calculates deltaT: FDV[1]=HF[1]-FDV[0]=TS_NOW - TS_LAST
						...
						#calculates ewma
						bebaparser.OFPExpActionSetDataVariable(table_id=2, opcode=bebaproto.OPCODE_EWMA, output_fd_id=3, operand_1_fd_id=3, operand_2_fd_id=2, operand_3_fd_id=1),
		...
		for s in UPPER_PORTS:
			for metadata in LEAVES:
				...
				actions_ewma = [#calculates deltaT: FDV[1]=HF[1]-FDV[0]=TS_NOW - TS_LAST
						...
						#calculates ewma
						bebaparser.OFPExpActionSetDataVariable(table_id=3, opcode=bebaproto.OPCODE_EWMA, output_fd_id=3, operand_1_fd_id=3, operand_2_fd_id=2, operand_3_fd_id=1),
						#saves current timestamp
	...
	def install_spines(self, datapath):
		...
		for i in LEAVES:
			...
			actions = [#calculates deltaT: FDV[1]=HF[1]-FDV[0]=TS_NOW - TS_LAST
						...
						#calculates ewma
						bebaparser.OFPExpActionSetDataVariable(table_id=0, opcode=bebaproto.OPCODE_EWMA, output_fd_id=3, operand_1_fd_id=3, operand_2_fd_id=2, operand_3_fd_id=1),
						#saves current timestamp
